...
import type { ArticleDetailResponse, ArticleVocabResponse, VocabLibraryResponse } from '@/lib/api'
...
function toggleWord(vocab: ArticleVocabResponse) {
...
async function loadReader() {
  ...
  try {
    ...
    const [levels, vocabLibraries] = await Promise.all([
      ...
      getVocabLibraries(article.value.languageCode),
    ])
        ...
        <header class="article-header fade-in">
          ...
          <h1 class="serif">
            <template v-for="(segment, index) in segmentText(article.title)" :key="index">
              <button v-if="segment.vocab" class="word title-word" :class="{ active: expandedVocabId === segment.vocab.articleVocabId }" @click="toggleWord(segment.vocab)">{{ segment.text }}</button>
        ...
        <div class="prose serif">
          <p v-for="(paragraph, paragraphIndex) in paragraphs" :key="paragraphIndex">
            <template v-for="(segment, segmentIndex) in segmentText(paragraph)" :key="segmentIndex">
              <button v-if="segment.vocab" class="word" :class="{ active: expandedVocabId === segment.vocab.articleVocabId }" @click="toggleWord(segment.vocab)">{{ segment.text }}</button>
              <template v-else>{{ segment.text }}</template>
            </template>
          </p>
        </div>
            ...
            <Transition name="word-detail">
              <div v-if="expandedVocabId === vocab.articleVocabId" class="word-detail">
                ...
                <div v-if="vocab.kana || vocab.us || vocab.uk" class="pronunciations">
                  ...
                  <span v-if="vocab.us">US {{ vocab.us }}</span>
                  <span v-if="vocab.uk">UK {{ vocab.uk }}</span>
                </div>
...
.article-header { margin-bottom: 50px; }.article-kicker { margin: 0 0 16px; color: var(--secondary); font-size: 11px; font-weight: 700; text-transform: uppercase; }.article-header h1 { margin: 0; color: var(--ink); font-size: 49px; line-height: 1.18; font-weight: 650; }.article-meta { display: grid; grid-template-columns: repeat(4, minmax(0, 1fr)); gap: 0; padding: 19px 0; margin: 30px 0 0; border-top: 1px solid var(--outline); border-bottom: 1px solid var(--outline); }.article-meta div { min-width: 0; padding: 0 16px; border-left: 1px solid var(--outline); }.article-meta div:first-child { padding-left: 0; border-left: 0; }.article-meta dt { color: var(--ink-muted); font-size: 10px; }.article-meta dd { overflow: hidden; margin: 4px 0 0; color: var(--ink); font-size: 12px; font-weight: 700; text-overflow: ellipsis; white-space: nowrap; }
.prose { color: #282a29; font-size: 18.5px; line-height: 1.78; }.prose p { margin: 0 0 30px; white-space: pre-wrap; }.word { display: inline; padding: 0 1px; border: 0; border-bottom: 1px solid var(--secondary); border-radius: 1px; color: inherit; background: rgba(253,203,155,.23); font: inherit; line-height: inherit; }.word:hover, .word.active { background: var(--secondary-soft); }.title-word { text-transform: none; }
