      ...
      <article class="flashcard surface">
        ...
        <div v-if="showMeaning" class="flash-meaning">
          <template v-if="parseTranslations(currentWord.translations).length">
            <div v-for="(item, index) in parseTranslations(currentWord.translations)" :key="`${item.type}-${index}`" class="flash-meaning-item">
              <span v-if="item.type">{{ item.type }}</span>
              <strong>{{ item.translation }}</strong>
            </div>
          </template>
          <strong v-else>暂无释义</strong>
        </div>
      </article>
...
.session-shell { width: min(100%, 720px); margin: 12px auto 0; }.session-heading { display: flex; align-items: end; justify-content: space-between; gap: 20px; }.session-heading .eyebrow { margin-bottom: 8px; }.session-heading .page-title { font-size: 36px; }.session-heading > strong { color: var(--ink-muted); font-size: 13px; font-weight: 650; }.session-progress { height: 5px; overflow: hidden; margin: 18px 0; border-radius: 4px; background: var(--surface-high); }.session-progress span { display: block; height: 100%; border-radius: inherit; background: var(--primary); transition: width .25s ease; }.flashcard { min-height: 380px; display: flex; align-items: center; flex-direction: column; padding: 42px 34px 34px; text-align: center; }.flash-term { margin-top: 34px; }.flash-term h2 { margin: 0; color: var(--primary); font-size: 48px; line-height: 1.2; }.pronunciation { display: flex; flex-wrap: wrap; justify-content: center; gap: 8px 18px; margin-top: 14px; color: var(--ink-muted); font-size: 13px; }.meaning-toggle { display: inline-flex; align-items: center; gap: 7px; margin-top: 34px; padding: 8px 12px; border: 1px solid var(--outline); border-radius: 6px; color: var(--primary); background: var(--surface-lowest); font-size: 12px; }.meaning-toggle:hover { border-color: var(--primary); background: var(--surface-low); }.flash-meaning { width: min(100%, 520px); display: grid; gap: 10px; padding-top: 20px; margin-top: 18px; border-top: 1px solid var(--outline); color: var(--ink); font-size: 14px; line-height: 1.6; }.flash-meaning-item { display: grid; grid-template-columns: minmax(34px, auto) 1fr; align-items: baseline; gap: 10px; text-align: left; }.flash-meaning-item > span { color: var(--secondary); font-size: 12px; font-weight: 750; }.flash-meaning-item > strong { font-weight: 600; }.rating-actions { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; margin-top: 18px; }.rating-btn { min-height: 70px; display: flex; align-items: center; justify-content: center; flex-direction: column; gap: 4px; border: 1px solid var(--outline); border-radius: 7px; background: var(--surface-lowest); transition: border-color .18s ease, background-color .18s ease, transform .18s ease; }.rating-btn:hover:not(:disabled) { transform: translateY(-1px); }.rating-btn:disabled { opacity: .55; cursor: wait; }.rating-btn span { font-size: 14px; font-weight: 700; }.rating-btn small { font-size: 10px; font-weight: 750; letter-spacing: .04em; }.rating-btn.unknown { color: var(--error); }.rating-btn.unknown:hover:not(:disabled) { border-color: var(--error); background: #f9e8e8; }.rating-btn.vague { color: var(--secondary); }.rating-btn.vague:hover:not(:disabled) { border-color: var(--secondary); background: var(--secondary-soft); }.rating-btn.known { color: var(--success); }.rating-btn.known:hover:not(:disabled) { border-color: var(--success); background: #e6efe8; }.session-state { min-height: 320px; display: flex; align-items: center; justify-content: center; flex-direction: column; gap: 9px; color: var(--ink-muted); text-align: center; }.session-state strong { color: var(--primary); font-size: 17px; }.session-state p { margin: 0; font-size: 13px; }.session-state .btn { margin-top: 8px; }.loading-line { width: 92px; height: 2px; overflow: hidden; position: relative; background: var(--surface-high); }.loading-line::after { position: absolute; inset: 0; content: ''; background: var(--primary); transform-origin: left; animation: loading 1s ease-in-out infinite; }@keyframes loading { 0% { transform: translateX(-100%) scaleX(.35); } 55% { transform: translateX(35%) scaleX(.65); } 100% { transform: translateX(110%) scaleX(.3); } }
