...
<template>
  <main class="page">
    ...
    <section class="recharge-section fade-in">
      ...
      <div class="amount-grid">
        <button v-for="amount in amounts" :key="amount" class="amount-card surface" :class="{ selected: selectedAmount === amount && customAmount === null }" type="button" @click="selectAmount(amount)">
          ...
          <CheckCircle2 v-if="selectedAmount === amount && customAmount === null" :size="17" />
        </button>
      </div>

      <div class="custom-row surface">
        <div><label class="field-label" for="custom-amount">自定义金额（1–100 元）</label><div class="amount-input"><span>¥</span><input id="custom-amount" v-model.number="customAmount" min="1" max="100" type="number" placeholder="输入金额" /></div></div>
        <div class="payment-due"><span>待支付金额</span><strong v-if="amountValid">¥{{ formatMoney(payableAmount) }}</strong><strong v-else>--</strong></div>
      </div>
    ...
    <Transition name="dialog">
      <div v-if="paymentDialogOpen" class="modal-backdrop" @click.self="closePaymentDialog">
        <section class="dialog-panel payment-modal surface" role="dialog" aria-modal="true" aria-labelledby="payment-dialog-title">
...
@media (max-width: 760px) { .header-actions { width: 100%; flex-wrap: wrap; }.balance-panel { grid-template-columns: 1fr; }.balance-panel > div { min-height: 120px; border-right: 0; border-bottom: 1px solid var(--outline); }.amount-grid { grid-template-columns: repeat(2, 1fr); }.custom-row { align-items: stretch; flex-direction: column; }.payment-due { min-width: 0; padding-inline: 0; }.records-modal, .payment-modal { padding: 23px 20px 18px; }.records-table { min-width: 700px; } }
</style>
