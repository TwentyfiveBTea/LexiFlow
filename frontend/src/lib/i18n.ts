...
const dynamicPatterns: Array<{ pattern: RegExp; render: (match: RegExpExecArray, locale: InterfaceLanguage) => string }> = [
  { pattern: /^Repository · (\d[\d,]*) texts$/, render: (m, l) => l === 'en' ? `Repository · ${m[1]} texts` : `ライブラリ · ${m[1]}件` },
  { pattern: /^Collection · (\d[\d,]*) words$/, render: (m, l) => l === 'en' ? `Collection · ${m[1]} words` : `単語帳 · ${m[1]}語` },
  { pattern: /^· Demo$/, render: (m, l) => l === 'ja' ? '· デモ' : '· Demo' },
  { pattern: /^(\d[\d,]*) 个词条$/, render: (m, l) => l === 'en' ? `${m[1]} words` : `${m[1]} 語` },
  { pattern: /^(\d[\d,]*) 个词汇已(复用|完成解析)$/, render: (m, l) => l === 'en' ? `${m[1]} words ${m[2] === '复用' ? 'reused' : 'analyzed'}` : `${m[1]}語を${m[2] === '复用' ? '再利用' : '解析'}` },
  { pattern: /^这是你注册的第 (\d+) 天$/, render: (m, l) => l === 'en' ? `Day ${m[1]} since joining` : `登録から${m[1]}日目` },
  { pattern: /^解析 (.+) 的词汇$/, render: (m, l) => l === 'en' ? `Analyze vocabulary in ${m[1]}` : `${m[1]} の語彙を解析` },
  { pattern: /^查看 (.+) 详细信息$/, render: (m, l) => l === 'en' ? `View details for ${m[1]}` : `${m[1]} の詳細を表示` },
  { pattern: /^查看 (.+) 统计$/, render: (m, l) => l === 'en' ? `View statistics for ${m[1]}` : `${m[1]} の統計を表示` },
  { pattern: /^从词汇库删除 (.+)$/, render: (m, l) => l === 'en' ? `Remove ${m[1]} from collection` : `${m[1]} を単語帳から削除` },
  { pattern: /^已加入“(.+)”$/, render: (m, l) => l === 'en' ? `Added to “${m[1]}”` : `「${m[1]}」に追加しました` },
  { pattern: /^欢迎回来，(.+)$/, render: (m, l) => l === 'en' ? `Welcome back, ${m[1]}` : `おかえりなさい、${m[1]}` },
  { pattern: /^(\d+) 个词待复习$/, render: (m, l) => l === 'en' ? `${m[1]} words due for review` : `復習待ちの単語 ${m[1]} 語` },
  { pattern: /^创建时间 · (.+)$/, render: (m, l) => l === 'en' ? `Created · ${m[1]}` : `作成日時 · ${m[1]}` },
  { pattern: /^(\d+) 分钟$/, render: (m, l) => l === 'en' ? `${m[1]} min` : `${m[1]}分` },
  { pattern: /^已加入 · (.+)$/, render: (m, l) => l === 'en' ? `Added · ${m[1]}` : `追加済み · ${m[1]}` },
  { pattern: /^(.+) 已上传，文章正在后台处理$/, render: (m, l) => l === 'en' ? `${m[1]} uploaded. The article is processing in the background.` : `${m[1]} をアップロードしました。記事をバックグラウンドで処理中です` },
  { pattern: /^订单 (.+) 已创建，请继续完成支付$/, render: (m, l) => l === 'en' ? `Order ${m[1]} created. Continue to payment.` : `注文 ${m[1]} を作成しました。支払いを続けてください` },
  { pattern: /^共 (\d[\d,]*) 条$/, render: (m, l) => l === 'en' ? `${m[1]} total` : `全${m[1]}件` },
  { pattern: /^(\d[\d,]*) 词$/, render: (m, l) => l === 'en' ? `${m[1]} words` : `${m[1]} 語` },
]
